dimred.pseudobulk <- function (object, lognorm = T, scale.factor = 10000, plot.vars = NULL, 
    ...
    scale.factor = 10000, verbose = T) 
{
    ...
    psbk <- dimred.pseudobulk(psbk, scale.factor = scale.factor, 
        lognorm = lognorm, plot.vars = c(cluster.var, group.vars))
...
{
    groups.df <- grouped.cells <- split(colnames(object), apply(object@meta.data[, 
    ...
    }) 
{
    ...
    if (keep_factors) {
        for (avar in annotation.vars) {
            if (!is.factor(meta.data[, avar])) {
                (next)()
            }
            if (length(unique(meta.data[, avar])) == 1) {
                sum.d[, avar] <- factor(sum.d[, avar], levels = levels(meta.data[, 
                  avar]))
            }
        }
    }
